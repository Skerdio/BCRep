report /*50105*/50008 "Sales - Credit MemoAL"
{
    DefaultLayout = RDLC;
    RDLCLayout = './Raportet/SalesCreditMemo.rdl';
    ...
    dataset
    {
        dataitem("Sales Cr.Memo Header"; "Sales Cr.Memo Header")
        {
            ...
            column(NIVF; NIVF)
            { IncludeCaption = true; }
            column(IICRef; IICRef)
            { }
            column(IICRefLbl; IICRefLbl)
            { }
            ...
            dataitem(CopyLoop; "Integer")
            {
                ...
                dataitem(PageLoop; "Integer")
                {
                    ...
                    dataitem("Sales Cr.Memo Line"; "Sales Cr.Memo Line")
                    {
                        ...
                        { }
                        column(Amount_Including_VAT; Format(-1 * "Amount Including VAT"))
                        {
                        ...
                        column(AmtBefDiscount; (-1 * AmtBefDiscount))
                        {
                            AutoFormatType = 1;
                            DecimalPlaces = 0 : 2;
                        }
                        ...
                        { }
                        column(LineAmt_SalesCrMemoLine; Round(-1 * "Line Amount", 0.01))
                        {
                            AutoFormatExpression = GetCurrencyCode();
                            AutoFormatType = 1;
                        }
                        ...
                        column(Qty_SalesCrMemoLine; -1 * Quantity)
                        {
                        }
                        ...
                        column(NNCTotalLineAmt; -1 * NNC_TotalLineAmount)
                        {
                            AutoFormatExpression = GetCurrencyCode();
                            AutoFormatType = 1;
                        }
                        column(NNCTotalAmtInclVat; -1 * NNC_TotalAmountInclVat)
                        {
                            AutoFormatExpression = GetCurrencyCode();
                            AutoFormatType = 1;
                        }
                        ...
                        column(NNCTotalAmt; -1 * NNC_TotalAmount)
                        {
                            AutoFormatExpression = GetCurrencyCode();
                            AutoFormatType = 1;
                        }
                        ...
                        begin
                            ...
                            NNC_TotalInvDiscAmount += "Inv. Discount Amount" * -1;
                            ...
                            VatPerc := -1 * ("Amount Including VAT" - "VAT Base Amount");
                            AmtBefDiscount += Round(Quantity * "Unit Price", 0.01);
            ...
            begin
                ...
                if "Sales Cr.Memo Header"."Currency Code" <> 'ALL' then
                    gAmount := "Sales Cr.Memo Header"."Amount Including VAT" * (1 / "Sales Cr.Memo Header"."Currency Factor")
                else
                    gAmount := "Sales Cr.Memo Header"."Amount Including VAT";
                //
                gInvoiceExRate := 1;
                if ("Sales Cr.Memo Header"."Currency Code" <> '') and ("Sales Cr.Memo Header"."Currency Code" <> 'ALL') then begin
                    if "Sales Cr.Memo Header"."Currency Factor" <> 0 then
                        gInvoiceExRate := round((1 / "Sales Cr.Memo Header"."Currency Factor"), 0.01)
                    else
                        gInvoiceExRate := 1;
                end;
                //
                if CompInfoESetup."FE Activated" then
                    if "Sales Cr.Memo Header"."FE Deactivate" = false then
                        InvNo := "Sales Cr.Memo Header"."Fiscalised Invoice number" + '/' + Format(Date2DMY(Today, 3))
                    else
                        InvNo := "Sales Cr.Memo Header"."No." + '/' + Format(Date2DMY(Today, 3));
                ...
                if "Sales Cr.Memo Header"."Is Simplified Inv" then
                    InvoiceTitle := SimplFiscalizedInvLbl
                else
                    InvoiceTitle := FiscalizedInvLbl;


                IICRef := "Sales Cr.Memo Header"."Inv IIC Refs";


                if "Sales Cr.Memo Header"."Is Summary" then begin
                    IsSumm := 1;
                    SummRefIIC := "Sales Cr.Memo Header"."Sum Inv IIC Refs";
                end;
                //
                Cust.Reset();
                ...
                // //
                SetBlob("Sales Cr.Memo Header".NSLF, CompInfoESetup."VAT Registration No.", "Sales Cr.Memo Header"."Fiscalization Date", "Sales Cr.Memo Header"."Fiscalised Invoice number", CompInfoESetup."Business Unit Code", CompInfoESetup."TCR Code", CompInfoESetup."Soft Code", gAmount);
    ...
    var
        ...
        SalesPersonLbl: Label 'Kodi agjentit';
        ...
        IICRef: Text;
        IICRefLbl: Label 'Invoice Reference';
    ...
    begin
        lQRCode := 'https://efiskalizimi-app-test.tatime.gov.al/invoice-check/#/verify';
        if pNslf <> '' then
            lQRCode += '?iic=' + pNslf;
        if pVatRegNo <> '' then
    ...
    begin
        ...
        ReturnOrderNoText := FormatDocument.SetText(SalesCrMemoHeader."Return Order No." <> '', SalesCrMemoHeader.FieldCaption(SalesCrMemoHeader."Return Order No."));
        ReferenceText := FormatDocument.SetText(SalesCrMemoHeader."Your Reference" <> '', SalesCrMemoHeader.FieldCaption(SalesCrMemoHeader."Your Reference"));
        VATNoText := FormatDocument.SetText(SalesCrMemoHeader."VAT Registration No." <> '', SalesCrMemoHeader.FieldCaption(SalesCrMemoHeader."VAT Registration No."));
        AppliedToText :=
