report /*50104*/50009 "Sales - InvoiceAL"
{
    DefaultLayout = RDLC;
    RDLCLayout = './Raportet/SalesInvoice - copy.rdl';
    ...
    dataset
    {
        dataitem("Sales Invoice Header"; "Sales Invoice Header")
        {
            ...
            column(NIVF; NIVF)
            { IncludeCaption = true; }
            column(IsSumm; IsSumm)
            { }
            column(SummRefLbl; SummRefLbl)
            { }
            column(SummRefIIC; SummRefIIC)
            { }
            column(IsCorrec; IsCorrec)
            { }
            column(CorrecRefLbl; CorrecRefLbl)
            { }
            column(CorrecRefIIC; CorrecRefIIC)
            { }
            column(Fiscalization_Date; "Fiscalization Date")
            { }
            column(FiscalizationDTLbl; FiscalizationDTLbl)
            { }
            column(temp; TM.Content)
            { }
            column(TCRCode; TCRCode)
            { }
            ...
            { }
            column(gInvoiceExRate; gInvoiceExRate)
            { }
            column(gInvoiceExRateLBl; gInvoiceExRateLBl)
            { }
            column(TCRCodeLbl; TCRCodeLbl)
            { }
            ...
            dataitem(CopyLoop; "Integer")
            {
                ...
                dataitem(PageLoop; "Integer")
                {
                    ...
                    { }
                    column(VatRegLBl; VatRegLBl)
                    { }
                    column(DueDate_SalesInvHdr; Format("Sales Invoice Header"."Due Date", 0, 4))
                    { }
                    column(SalesPersonText; SalesPersonLbl/*SalesPersonText*/)
                    { }
                    ...
                    dataitem("Sales Invoice Line"; "Sales Invoice Line")
                    {
                        ...
                        column(Amount_Including_VATLbl; Amount_Including_VATLbl)
                        { }
                        column(Amount_Including_VAT; Format("Amount Including VAT"))
                        {
                        ...
                        column(AmtBefDiscount; Round(AmtBefDiscount, 0.01))
                        {
                            AutoFormatType = 1;
                            DecimalPlaces = 0 : 2;
                        }
                        column(AmtBefDiscountLbl; AmtBefDiscountLbl)
                        { }
                        column(DiscountAmt; (-1 * DiscountAmt))
                        {
                            AutoFormatType = 1;
                            DecimalPlaces = 0 : 2;
                        }
                        column(DiscountAmtLbl; DiscountAmtLbl)
                        { }
                        ...
                        begin
                            ...
                            VatPerc := "Amount Including VAT" - "VAT Base Amount";
                            AmtBefDiscount += Quantity * "Unit Price";
                            DiscountAmt += Round("Line Discount Amount", 0.01);
                    ...
                    {
                        DataItemTableView = SORTING(Number);
                        column(VatIDCounter; VATAmountLine."VAT Id. Counter")
                        {
                            AutoFormatType = 1;
                        }
                        column(VatIdCounterLbl; VatIdCounterLbl)
                        { }
            ...
            begin
                ...
                if "Sales Invoice Header"."Currency Code" <> 'ALL' then
                    gAmount := "Sales Invoice Header"."Amount Including VAT" * (1 / "Sales Invoice Header"."Currency Factor")
                else
                    gAmount := "Sales Invoice Header"."Amount Including VAT";
                //
                gInvoiceExRate := 1;
                if ("Sales Invoice Header"."Currency Code" <> '') and ("Sales Invoice Header"."Currency Code" <> 'ALL') then begin
                    if "Sales Invoice Header"."Currency Factor" <> 0 then
                        gInvoiceExRate := round((1 / "Sales Invoice Header"."Currency Factor"), 0.01)
                    else
                        gInvoiceExRate := 1;
                end;
                //
                if CompInfoESetup."FE Activated" then
                    if "Sales Invoice Header"."FE Deactivate" = false then
                        InvNo := "Sales Invoice Header"."Fiscalised invoice number" + '/' + Format(Date2DMY(Today, 3))
                    else
                        InvNo := "Sales Invoice Header"."No." + '/' + Format(Date2DMY(Today, 3));

                if "Sales Invoice Header"."Is Simplified Inv" then
                    InvoiceTitle := SimplFiscalizedInvLbl
                else
                    InvoiceTitle := FiscalizedInvLbl;

                if "Sales Invoice Header"."Is Summary" then begin
                    IsSumm := 1;
                    SummRefIIC := "Sales Invoice Header"."Sum Inv IIC Refs";
                end;

                if "Sales Invoice Header"."Corrective/Is Bad Debt Invoice" = "Sales Invoice Header"."Corrective/Is Bad Debt Invoice"::"Is Corrective" then begin
                    IsCorrec := 1;
                    CorrecRefIIC := "Sales Invoice Header"."Inv IIC Refs";
                end;
                //
                if ("Sales Invoice Header"."Work Description".HasValue) then begin
                    Notes := Format(GetWorkDescription());
                end;
                Cust.Reset();
                ...
                case Cust.IDType of
                    ...
                    Cust.IDType::TAX:
                        begin
                            VatRegNo := Cust."Tax No."
                        end;
                end;
                // 
                if ("Sales Invoice Header"."Invoice Type" = "Sales Invoice Header"."Invoice Type"::CASH) then
                    isCash := 1
                else
                    isCash := 0;

                SetBlob("Sales Invoice Header".NSLF, CompInfoESetup."VAT Registration No.", "Sales Invoice Header"."Fiscalization Date", "Sales Invoice Header"."Fiscalised Invoice number", "CompInfoESetup"."Business Unit Code", "CompInfoESetup"."TCR Code", "CompInfoESetup"."Soft Code", gAmount/*"Sales Invoice Header"."Amount Including VAT"*/);//
            end;
    ...
    var
        ...
        SalesPersonLbl: Label 'Agent Code';
        OperatorCode: Text;
        BusUnitCode: Text;
        TCRCode: Text;
        SoftCode: Text;
        ...
        TCRCodeLbl: Label 'TCR';
        TM: Record "Tenant Media";
        gAmount: Decimal;
        gInvoiceExRate: Decimal;
        gInvoiceExRateLbl: Label 'Exchange rate';//'Kursi kembimit';
        InvoiceTypeLbl: Label 'Invoice Type';//'Tipi fatures';
        InvNoLbl: Label 'Invoice Number';
        InvNo: Code[20];
        InvoiceTitle: Text;
        FiscalizedInvLbl: Label 'Fiscalized Invoice';
        SimplFiscalizedInvLbl: Label 'Simplified Fiscalized Invoice';
        PaymentMethodLbl: Label 'Payment method code';
        FiscalizationDTLbl: Label 'Fiscalization Date';
        CurrencyCodeLbl: Label 'Currency Code';
        Amount_Including_VATLbl: Label 'Amount Incl. VAT';
        VatPerc: Decimal;
        VatIdCounterLbl: Label 'No.';
        AmtBefDiscount: Decimal;
        AmtBefDiscountLbl: Label 'Amount Bef. Disocunt';
        DiscountAmt: Decimal;
        DiscountAmtLbl: Label 'Discount Amount';
        SummRefLbl: Label 'Summary References';
        SummRefIIC: Text[500];
        IsSumm: Decimal;
        CorrecRefLbl: Label 'NSLF References';
        CorrecRefIIC: Text[500];
        IsCorrec: Decimal;
        VatRegNo: Text;
        VatRegLBl: Label 'Buyer Tax Number';
        isCash: Decimal;
        NotesLbl: Label 'Notes';
        Notes: Text[100];


    procedure SetBlob(pNslf: Code[32];
                      pVatRegNo: Text[20];
                      pFiscDate: Text[65];
                      pInvNo: Code[20];
    ...
    begin
        lQRCode := 'https://efiskalizimi-app-test.tatime.gov.al/invoice-check/#/verify'; // duhet te shtohet dhe nr i fatures ne rastet kur fiskalziohet pa internet
        if pNslf <> '' then
            lQRCode += '?iic=' + pNslf;
        if pVatRegNo <> '' then
        ...
        // if pAmount <> 0 then
        lQRCode += '&prc=' + Format(round(pAmount, 0.01));

        lQRCode := TypeHelper.UrlEncode(lQRCode);
        lArgumentURL := StrSubstNo('https://api.qrserver.com/v1/create-qr-code/?data=%1', lQRCode);//https://api.qrserver.com/v1/create-qr-code/?size=150x150&data=%1
    ...
    begin
        ...
        OrderNoText := FormatDocument.SetText(SalesInvoiceHeader."Order No." <> '', SalesInvoiceHeader.FieldCaption(SalesInvoiceHeader."Order No."));
        ...
        VATNoText := FormatDocument.SetText(SalesInvoiceHeader."VAT Registration No." <> '', SalesInvoiceHeader.FieldCaption(SalesInvoiceHeader."VAT Registration No."));
